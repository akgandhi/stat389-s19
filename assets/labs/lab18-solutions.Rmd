...
```{r}
x28 <- read_rds("../notes/image_data/fashion_10_x28.rds")
```
...
X_train <- X[fmnist$train_id == "train",]
y_train <- to_categorical(y[fmnist$train_id == "train"])
```
...
tapply(y == pred, fmnist$train_id, mean)
```

```{r}
table(y[fmnist$train_id == "train"], pred[fmnist$train_id == "train"])
```
