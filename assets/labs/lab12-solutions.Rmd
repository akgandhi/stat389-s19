...
```{r}
model <- gam(median_house_value ~ latitude + longitude,
             data = housing,
             subset = train_id == "train")

pred <- predict(model, newdata = housing)
sqrt(tapply((housing$median_house_value - pred)^2, housing$train_id, mean))
```
...
```{r}
model <- gam(median_house_value ~ s(latitude) + s(longitude),
             data = housing,
             subset = train_id == "train")
...
```{r}
model <- gam(median_house_value ~ s(latitude, longitude),
             data = housing,
             subset = train_id == "train")
...
{
  pred_valid <- knn.reg(X_train, X_valid, y_train, k=k)$pred
...
```{r}
model <- gam(median_house_value ~ s(latitude, longitude) + s(mean_household_income, median_household_income),
             data = housing,
             subset = train_id == "train")

housing <- mutate(housing, pred = predict(model, newdata = housing))
sqrt(tapply((housing$median_house_value - housing$pred)^2, housing$train_id, mean))
```
