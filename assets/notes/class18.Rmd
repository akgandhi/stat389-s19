...
A commen alternative stores an additional momentum vector:

$$ \Delta w_{new} \leftarrow \mu \cdot \Delta w_{old} - \rho \cdot \nabla_w f $$
$$ w_{new} \leftarrow w_{old} + \Delta w_{new} $$

This allows us to approximate the hessian matrix while only storing an extra p variables
(rather than p squared). To implement in R we can do this:
